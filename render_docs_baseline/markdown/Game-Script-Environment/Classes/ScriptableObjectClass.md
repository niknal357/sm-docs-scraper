...
**Script template:** [View starter script](ScriptableObjectClass-Template.md)
...
Can receive events sent with [sm.event.sendToScriptableObject](../Static-Functions/sm.event.md#sendtoscriptableobject).

## Fields

| Name | Type | Description |
| --- | --- | --- |
| `scriptableObject` | [ScriptableObject](../Userdata/ScriptableObject.md) | The [ScriptableObject](../Userdata/ScriptableObject.md) belonging to this class instance. |
...
| `storage` | [Storage](../Userdata/Storage.md) | (Server side only, if [isSaveObject](#issaveobject) is enabled) A [Storage](../Userdata/Storage.md) object that can be used to store data for the next time loading this object after being unloaded. |
| `data` | any | Data from the "data" json element. |
| `params` | any | Parameter sent to [sm.scriptableObject.createScriptableObject](../Static-Functions/sm.scriptableObject.md#createscriptableobject). |
...
If enabled, the [ScriptableObject](../Userdata/ScriptableObject.md) will be recreated when loading a game. Otherwise, the [ScriptableObject](../Userdata/ScriptableObject.md) is considered a temporary object.
...
``` { .lua .api-signature }
ScriptableObjectClass:server_onCreate(  )
ScriptableObjectClass:client_onCreate(  )
```
...
``` { .lua .api-signature }
ScriptableObjectClass:server_onDestroy(  )
...
ScriptableObjectClass:client_onRefresh(  )
```
...
``` { .lua .api-signature }
ScriptableObjectClass:server_onFixedUpdate( timeStep )
ScriptableObjectClass:client_onFixedUpdate( timeStep )
```
...
``` { .lua .api-signature }
ScriptableObjectClass:server_onReceiveUpdate(  )
```
...
Called when the [ScriptableObject](../Userdata/ScriptableObject.md) is unloaded when exiting the game.
...
``` { .lua .api-signature }
ScriptableObjectClass:client_onUpdate( deltaTime )
```
...
``` { .lua .api-signature }
ScriptableObjectClass:client_onClientDataUpdate( data, channel )
```
...
``` { .lua .api-signature }
ScriptableObjectClass:client_onLocalPlayerChangedWorld( world )
```
